...
def Malaria():
    ...
    #cursor = db.connection.cursor(MySQLdb.cursors.DictCursor) 
    cursor = conn.cursor() 
    cursor.execute("select Tehsil,Result_txt from malaria WHERE Result_txt = %s ", (Malaria_P,) ) 
    ...
    print(total_postive_list)
    print(total_cases)

    # insert data into Database from csv to msql
    # load data
    import numpy as np
    import pandas
    import matplotlib.pyplot as plt
    variables = pandas.read_csv('Malyria.csv')
#print(result)
    #for row in variables:
        #cursor.execute('INSERT INTO malaria(MRNO,Age,Gender,District,Tehsil,Report_verified,CPT_ID_Result_Value,Result,CPT_ID,Result_txt)' 'VALUES("%s","%s", "%s","%s","%s","%s","%s","%s","%s","%s")', row)
    #conn.commit()
   # cursor.close()
    #print("Done") 
    ...
    return render_template('index.html', y=teshil_list, x=total_postive_list, Total_p=Malaria_mytehsil_Cases_1, Total_pred=Malaria_pred_Cases_1, Total_Malaria_Negative_Case2=Malaria_Negative_Cases_1, Total_Malaria_tested_Case1=Malaria_tested, Total_Malaria_Postive_Case2=Malaria_Postive_Cases_1)
